...
# Purpose: Restart all WordPress containers to apply secure port bindings
###############################################################################
...
echo -e "${GREEN}=== Restarting WordPress Containers ===${NC}"
...
for site in "${SITES[@]}"; do
    ...
    if docker-compose up -d --force-recreate 2>&1 | grep -q "Started\|done\|up-to-date"; then
...
echo -e "  Run: sudo netstat -tlnp | grep docker-proxy"
