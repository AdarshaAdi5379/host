...
def _queue_reconcile_for_instances(
...
def _set_no_store_headers(response: Response) -> Response:
    # Compute control-plane reads must always reflect the latest state.
...
class ComputeImageViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        # Catalog visibility is shared for all authenticated users.
        # Mutating actions remain admin-only via _require_admin().
        return ComputeImage.objects.all().order_by("name", "-created_at")
...
class ComputeFlavorViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        # Catalog visibility is shared for all authenticated users.
        # Mutating actions remain admin-only via _require_admin().
        return ComputeFlavor.objects.all().order_by("vcpu", "memory_mb", "disk_gb")
...
class SSHKeyPairViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        # Admins see all keys; regular users only see their own keys.
        if _is_admin_user(self.request.user):
            return (
                SSHKeyPair.objects.all().select_related("owner").order_by("-created_at")
            )
        return (
            SSHKeyPair.objects.filter(owner=self.request.user)
    ...
    def generate(self, request):
        ...
        ).strip() or f"{request.user.username}@hostpanel"
...
class SecurityGroupViewSet(viewsets.ModelViewSet):
    ...
    @action(
        detail=True, methods=["patch", "delete"], url_path="rules/(?P<rule_id>[^/.]+)"
    )
    def rule_detail(self, request, pk=None, rule_id=None):
        ...
        if request.method == "DELETE":
            ...
            running_instances = list(
                group.instances.filter(state="running").order_by("id")
            )
            queued = _queue_reconcile_for_instances(
                running_instances,
                requested_by=request.user,
                ...
                extra={
                    ...
                    "security_group_rule_id": int(rule_id),
                },
            )
...
class ComputeInstanceViewSet(viewsets.ModelViewSet):
    queryset = ComputeInstance.objects.all().select_related(
        "owner", "image", "flavor", "ssh_key"
    )
    ...
    def create(self, request, *args, **kwargs):
        ...
        current = (
            ComputeInstance.objects.filter(owner=request.user)
    ...
    def set_security_groups(self, request, pk=None):
        ...
        groups = list(
            SecurityGroup.objects.filter(owner=instance.owner, id__in=parsed_ids)
        )
    ...
    def latest_operation(self, request, pk=None):
        ...
        return Response(
            {"latest_operation": ComputeOperationSerializer(op).data if op else None}
        )
...
class ComputeOperationViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        qs = (
            ComputeOperation.objects.all()
            .select_related("instance", "requested_by")
            .order_by("-created_at")
        )
        if _is_admin_user(self.request.user):
            return qs
...
class ComputeEventViewSet(viewsets.ReadOnlyModelViewSet):
    queryset = ComputeEvent.objects.all().select_related(
        "instance", "operation", "created_by"
    )
