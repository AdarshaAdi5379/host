...
TEMPLATE_PATH="${BACKEND_DIR}/deploy/systemd/host-django-api.service.template"

SERVICE_NAME="${SERVICE_NAME:-host-django-api}"
...
if [[ "${EUID}" -ne 0 ]]; then
  ...
  echo "  sudo SERVICE_NAME=${SERVICE_NAME} RUN_AS_USER=${RUN_AS_USER} PYTHON_BIN='${PYTHON_BIN}' ${SCRIPT_DIR}/install_django_api_service.sh" >&2
  exit 1
fi
